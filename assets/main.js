...
$(function(){
    ...
    function  cityDefinition (){
        if ($.cookie('NameCity') == undefined || $.cookie('NameCity') == null )
        {
          ymaps.ready(function(){
            ...
            if (geolocation.city=='' || geolocation.city== undefined || geolocation.city== null || geolocation.city== 'Не определен' )
            {
    ...
    $.getJSON('/assets/areas.json', function(data) {
      ...
      $.each(arrayCity, function() {
        $('#select_city').prepend('<option value="'+ this.value +'">'+ this.name +'</option>');
      })
       
      //Запись значения начального поисковик
      $('.js-select2').chosen(
        $('#select_city').val(cookieCityVal)
      )
      ...
      $.each(array, function() {
        $('#region ul').prepend('<li value="'+ this.value +'" >'+ this.name +'</li>');
      })
      
      
      //Формирует первый список при загрузке
      ValRegion = $('#region ul li:first').val();
      NameRegion = $('#region ul li:first').text();
        
      $('#region li:first').addClass('active');
      $('#city').prepend('<ul value="'+ ValRegion+'"><li value="0"></li></ul>');
      $.each(data, function(i, el) {
        if(el.id == ValRegion){
          $.each(el.areas, function(i, c){
            if (c.parent_id == ValRegion)
            {
              $('#city ul').append('<li value="'+ c.id +'" >'+ c.name +'</li>');
            }
          })
        }
      });
      ...
      $('.saveCity').on('click', function(){
        ...
        cookieCityName = $('#select_city option[value="'+ cookieCityVal +'"]').text();
